...
TFLM C++ 라이브러리는 기본적으로는 TensorFlow Lite와 호환되도록 설계된 구조를 통해 접근하되 마이크로컨트롤러 한정 라이브러리로서 제한된 기능으로 추가나 삭제 또는 재정의된다.
...
위 함수는 micro_interpreter.h에 선언된 MicroInterpreter class에 속 있는 메소드로 아래와 같은 순서로 수행된다.
