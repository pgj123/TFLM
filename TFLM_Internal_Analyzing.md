...
위 그림은 memory arena의 구조를 표현하는 다이어그램이다. 마이크로컨트롤러 기반 기계학습 추론에 있어 효율적인 에너지 이용을 위한 방법은 에너지 하베스팅을 통한 전원 공급 상황을 전제로 하는 경우 고려해야 하는 연구사항 중 하나이지만, 마이크로컨트롤러가 갖는 특수한 메모리 자원의 제한사항 속에서 메모리 공간을 효율적으로 사용하는 방법은 특정 연구 상황에 종속되지 않는 중요한 이슈이며 올바르게 이해한 바에 따라 디자인 구조 및 설계가 이루어져야 한다.  TFLM에서 추론을 수행하기 위한 준비과정의 내부 구조 흐름을 기술하는 목적 역시 현재 TFLM에서 설계한 메모리 공간(특히 SRAM) 활용 방법을 
